...
function postcssLinter(location, compilation, saver, timestamp) {
        ...
        if(!err) {
          ...
          npmRun.exec('npm run build', {cwd: currwd}, function (err, stdout, stderr) {
            if(!err) {
              ...
              fs.copySync(`${currwd}/dist`, `${process.cwd()}${location}`)
              console.log(`Dashboard copied to: ${process.cwd()}${location}`)
